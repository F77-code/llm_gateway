...
def _build_xai_body(request: ChatCompletionRequest) -> dict[str, Any]:
    body: dict[str, Any] = {
        "model": request.model,
...
def _xai_response_to_openai(data: dict[str, Any]) -> ChatCompletionResponse:
    if not isinstance(data, dict):
        raise ProviderError("xAI returned a non-object JSON response.")

    choices_raw = data.get("choices")
    if not isinstance(choices_raw, list) or not choices_raw:
        raise ProviderError("xAI returned no choices.")
    ...
    if not choices:
        raise ProviderError("xAI choices could not be parsed.")
...
class XAIProvider(BaseLLMProvider):
    """
    xAI Grok Chat Completions API (`/v1/chat/completions`).

    Supports tool calling, ``reasoning_effort`` for reasoning models,
    and ``search_parameters`` for live-search augmentation.
    Provider-specific request parameters are forwarded from ``model_extra``.
    """
    ...
    async def chat_completion(
        ...
        if request.stream:
            raise ProviderError(
                "XAIProvider does not support stream=true; use a non-streaming request.",
            )

        url = f"{self._base_url}/chat/completions"
        payload = _build_xai_body(request)
        ...
        data = response.json()
        return _xai_response_to_openai(data)
