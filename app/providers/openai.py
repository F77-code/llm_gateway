...
def _build_openai_body(request: ChatCompletionRequest) -> dict[str, Any]:
    body: dict[str, Any] = {
        "model": request.model,
...
def _openai_response_to_chat(data: dict[str, Any]) -> ChatCompletionResponse:
    if not isinstance(data, dict):
        raise ProviderError("OpenAI returned a non-object JSON response.")
    ...
    if not choices:
        raise ProviderError("OpenAI choices could not be parsed.")
...
class OpenAIProvider(BaseLLMProvider):
    ...
    async def chat_completion(
        ...
        if request.stream:
            raise ProviderError(
                "OpenAIProvider does not support stream=true; use a non-streaming request.",
            )

        url = f"{self._base_url}/chat/completions"
        payload = _build_openai_body(request)
        ...
        data = response.json()
        return _openai_response_to_chat(data)
